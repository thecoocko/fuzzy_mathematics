...
n = int(input("SIZE: "))
r_1 = [[1, 0.9,0.8,0.7,1],[0.6,1,0.7,0.3,0.7],[0.3,0,0.2,0.5,0.1],[0.3,0.5,0.3,1,0.4],[0.3,0.6,0.5,0.7,1]]
#[[float(input(f"R1[{i}][{j}]: ")) for i in range(n)]  for j in range(n)]
r_2 = [[1,0,1,0,0.6],[0.1,1,0.5,0.4,0.3],[0.5,0.7,1,0,0.7],[0.5,0.3,0,1,0.7],[0.7,0.9,0.8,0.5,1]]#[[float(input(f"R2[{i}][{j}]: ")) for i in range(n)]  for j in range(n)]
#r1 [[1,0.5,0.3],[0,1,0.8],[1,0.5,1]]
#r2 [[1,0.1,0],[0.3,1,1],[1,0.5,1]]
lambda1 = 0.2#float(input(f"lambda1: "))
lambda2 = 0.8#float(input(f"lambda2: "))
...
def main():
    ...
    print(f"Q1 {intersectionOfInitialRelations(r_1,r_2)}")
    print(f"Q2 {additiveConvolutionOfRelations(r_1,r_2)}")
